...
// Feistel encryption
uint64_t feistel_encrypt(uint64_t plaintext, const uint64_t *round_keys) {
    uint32_t L = plaintext >> 32;                // Left half (upper 32 bits)
    uint32_t R = plaintext & 0xFFFFFFFFU;        // Right half (lower 32 bits)

    for (int i = 0; i < ROUNDS; i++) {
        uint32_t temp = R;
        R = L ^ func(R, round_keys[i]);     // R_new = L_old XOR F(R_old, K_i)
        L = temp;                           // L_new = R_old
    }
    // Swap halves before combining
    return ((uint64_t)R << 32) | L;
}
// Feistel decryption
uint64_t feistel_decrypt(uint64_t ciphertext, const uint64_t *round_keys) {
    uint32_t L = ciphertext >> 32;               // Left half (upper 32 bits)
    uint32_t R = ciphertext & 0xFFFFFFFFU;       // Right half (lower 32 bits)

    for (int i = 0; i < ROUNDS; i++) {
        // Decryption uses round keys in reverse order
        uint32_t temp = L;
        L = R ^ func(L, round_keys[ROUNDS - 1 - i]); // L_new = R_old XOR F(L_old, K_i)
        R = temp;                           // R_new = L_old
    }

    // Swap halves before combining to restore original order
    return ((uint64_t)L << 32) | R;
}

// (E-box): 32 bits -> 48 bits
uint64_t E_box(uint32_t R) {
    uint64_t out = 0ULL;

    /* Note: GETBIT32 uses positions 1..32 where 1 is MSB of the 32-bit word. */
...
// Permutation (P-box): 32 bits -> 32 bits
uint32_t P_box(uint32_t in) {
    uint32_t out = 0U;
...
Generate_Subkeys(Main_Key: 64-bit block)
    ...
    // 2. تقسيم الـ 56 بت إلى نصفين C0 و D0
    C_i = Key_56_bit[0:28] // النصف الأيسر
    D_i = Key_56_bit[28:56] // النصف الأيمن

    // تعريف عدد الإزاحات لكل جولة
    Shift_Amounts = [1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1]
    Subkeys = Array_of_16_Subkeys

    FOR i FROM 1 TO 16 DO
        // 3. الإزاحة الدوارنية اليسرى (Circular Left Shift)
        Shift_Count = Shift_Amounts[i-1]
        C_i+1 = Left_Circular_Shift(C_i, Shift_Count)
        D_i+1 = Left_Circular_Shift(D_i, Shift_Count)

        // دمج النصفين لإنتاج المفتاح المرشح للجولة
        Key_Candidate_56_bit = Concatenate(C_i+1, D_i+1)

        // 4. التبديل الاختياري الثاني (PC-2): يقلل المفتاح من 56 إلى 48 بت
        K_i = Apply_Permutation(Key_Candidate_56_bit, PC_2_Table)

        // تخزين المفتاح الفرعي
        Subkeys[i] = K_i

        // تحديث C و D للجولة التالية
        C_i = C_i+1
        D_i = D_i+1
    END FOR

    RETURN Subkeys // مصفوفة الـ 16 مفتاحاً فرعياً
END FUNCTION
*/
/*
To be implemented
